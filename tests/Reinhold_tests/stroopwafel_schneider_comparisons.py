...
output_dirs  =  [  'hurley',      'belczynski',      'fryer',       'muller',      'mullermandel',  'schneider_std',  'schneider_alt'     ]
compas_mods  =  [  'HURLEY2000',  'BELCZYNSKI2002',  'FRYER2012',   'MULLER2016',  'MULLERMANDEL',  'SCHNEIDER2020',  'SCHNEIDER2020ALT'  ]
compas_kick  =  [  'MAXWELLIAN',  'MAXWELLIAN',      'MAXWELLIAN',  'MAXWELLIAN',  'MULLERMANDEL',  'MULLERMANDEL',   'MULLERMANDEL'      ]

whichmodel = int(sys.argv[1])  # take as input val later

### Set default stroopwafel inputs - these are overwritten by any command-line arguments

num_systems = 1000000               # Number of binary systems to evolve                                  
#output_folder = 'output/'           # Location of output folder (relative to cwd)                         
output_folder = '/home/rwillcox/output_oz101/schneider_project2/' + output_dirs[whichmodel]           # Location of output folder (relative to cwd)                         
random_seed_base = np.random.randint(0, 100) # The initial random seed to increment from                           
...
def update_properties(locations, dimensions):
    ...
    for location in locations:
        ...
        location.properties['--remnant-mass-prescription'] = compas_mods[whichmodel] #(options: [HURLEY2000, BELCZYNSKI2002, FRYER2012, MULLER2016, MULLERMANDEL, SCHNEIDER2020, SCHNEIDER2020ALT], default = FRYER2012)
        #location.properties['--fryer-supernova-engine'] = 'DELAYED' #(options: [DELAYED, RAPID], default = DELAYED)
        location.properties['--kick-magnitude-distribution'] = compas_kick[whichmodel] #(options: [ZERO, FIXED, FLAT, MAXWELLIAN, BRAYELDRIDGE, MULLER2016, MULLER2016MAXWELLIAN, MULLERMANDEL], default = MAXWELLIAN)
