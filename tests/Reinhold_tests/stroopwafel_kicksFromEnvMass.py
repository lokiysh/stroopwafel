...
which1 = 4
which0 = 0
massLossCategory = [ 'completeMassLoss', 'intermediateMassLoss', 'noMassLoss'][which0]
ecsnKickMagnitude = [ 0, 10, 30, 50, 100][which1]

num_systems = 100000                # Number of binary systems to evolve                                  
output_folder = '/home/rwillcox/astro/compas/COMPAS/outputs/ecsnEnvMassNatalKickVarieties/'+massLossCategory+'/kick-'+str(ecsnKickMagnitude) # Location of output folder (relative to cwd)                         
random_seed_base = (which1*10+which0)*num_systems  # The initial random seed to increment from                           
...
"""
command_line_args = {
        '--kick-magnitude-sigma-ECSN' : ecsnKickMagnitude, 
        '--mode' : 'BSE',
        '--logfile-type' : 'HDF5',
        '--rlof-printing' : 'TRUE',
...
def update_properties(locations, dimensions):
    ...
    for location in locations:
        ...
        # Get a Maxwellian kick which is greater than 50 km/s
        kick1 = 0
        while (kick1 < 50):
            kick1 = ss.maxwell.rvs(scale=265)

        kick2 = 0
        while (kick2 < 50):
            kick2 = ss.maxwell.rvs(scale=265)
            
        location.properties['--kick-magnitude-1'] = kick1
        location.properties['--kick-magnitude-2'] = kick2
