...
output_folder = '/home/rwillcox/output_oz101/nsk_Models/doubMaxwVIC/' # Location of output folder (relative to cwd)                         
random_seed_base = 22345            # The initial random seed to increment from                           
...
def update_properties(locations, dimensions):
    ...
    for location in locations:
        ...
        s2 = 316
        w = 0.4
        maxw1 = ss.maxwell.rvs(scale=s1, size=2)
        maxw2 = ss.maxwell.rvs(scale=s2, size=2)
        rnd = np.random.rand(2)
        location.properties['--kick-magnitude-1'] = maxw1[0] if (rnd[0]<w) else maxw2[0]
        location.properties['--kick-magnitude-2'] = maxw1[1] if (rnd[1]<w) else maxw2[1]
