...
def create_dimensions():
    ...
    """
    m1 = classes.Dimension('Mass_1', 45, 50, sampler.kroupa, prior.kroupa)
    q = classes.Dimension('q', 0.2, 1, sampler.uniform, prior.uniform, should_print = False)
    a = classes.Dimension('Separation', 1, 100, sampler.flat_in_log, prior.flat_in_log)
...
def configure_code_run(batch):
    ...
    compas_args = [compas_executable, "--grid", '"' + grid_filename + '"', '--outputPath', '"' + output_folder + '"', '--logfile-type', 'HDF5', '--output-container', output_container, '--random-seed', np.random.randint(2, 2**63 - 1)]
...
if __name__ == '__main__':
    ...
    genais_object = genais.Genais(TOTAL_NUM_SYSTEMS, NUM_CPU_CORES, NUM_SYSTEMS_PER_RUN, output_folder, output_filename, debug = debug, run_on_helios = run_on_helios, mc_only = mc_only)
    ...
    genais_object.initialize(dimensions, interesting_systems, configure_code_run, rejected_systems, update_properties_method = update_properties)

    intial_pdf = distributions.InitialDistribution(dimensions)
    #STEP 4: Run the 4 phases of stroopwafel
    genais_object.explore(intial_pdf) #Pass in the initial distribution for exploration phase
    genais_object.adapt(n_dimensional_distribution_type = distributions.Gaussian) #Adaptaion phase, tell stroopwafel what kind of distribution you would like to create instrumental distributions
    genais_object.refine(n_dimensional_distribution_type = distributions.Gaussian) #Stroopwafel will draw samples from the adapted distributions
    genais_object.calculate_weights_of_samples()
    end_time = time.time()
