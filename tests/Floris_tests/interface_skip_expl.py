...
sys.path.append('/home/floris/COMPASv2/COMPAS/postProcessing/Folders/CosmicIntegration/PythonScripts')
from stroopwafel_h5 import sw, genais, classes, prior, sampler, distributions, constants, utils, makeh5
import argparse
...
num_systems = 200000                  # Number of binary systems to evolve                                              # Note: overrides pythonSubmit value
output_folder = 'test2/'             # Location of output folder (relative to cwd)                                     # Note: overrides pythonSubmit value
# expl_dir = '/home/floris/Desktop/data/bbh_gen3_500k/'
random_seed_base = random.randint(0,2**32-1)               # The initial random seed to increment from                                       # Note: overrides pythonSubmit value

num_cores = 10                       # Number of cores to parallelize over
num_per_core = 2000                  # Number of binaries per batch
...
def create_dimensions():
    ...
    Z = classes.Dimension('--metallicity', 1e-4, 0.023, sampler.flat_in_log, prior.flat_in_log)
...
def interesting_systems(batch):
    ...
    if createh5:
        ...
        sp.rename(columns = lambda x: x.strip(), inplace = True)
        seeds = sp['SEED']
        for index, sample in enumerate(batch['samples']):
            seed = seeds[index]
            sample.properties['SEED'] = seed
    ...
    merge_mask = dco['Merges_Hubble_Time'] == 1
    dns = dco[np.logical_and(merge_mask, dco_mask)]
...
def bias_CI(output_folder, genais=False, adapt=False):
    """
    This is not a mandotory function. It takes feedback from the cosmic integration
    to further improve the efficiency of the AIS
    """

    #try:
    import fastCI
    # rate_det,_,_,_,_ = fastCI.find_detection_rate(output_folder,filename='COMPAS_output.h5', weight_column='mixture_weight')
    # rate_per_sys = np.sum(rate_det, axis=1)
    # print(COMPAS.sw_weights)
    #rate_per_sys[rate_per_sys == 0] = np.min(rate_per_sys[np.nonzero(rate_per_sys)])
    #print(min(rate_per_sys))
    #rate_per_sys = (np.log10(rate_per_sys) + abs(min(np.log10(rate_per_sys))))**3
    # mean = np.mean(rate_per_sys)
    #print(rate_per_sys)
    #print(mean)
    if adapt == True:
        rate_det,_,_,_,_ = fastCI.find_detection_rate(output_folder,filename='COMPAS_output.h5', weight_column='mixture_weight')
        ...
        if genais == False:
            for index, distribution in enumerate(sw.adapted_distributions):
                distribution.biased_weight = rate_per_sys[index] / mean
            #print(distribution.biased_weight)
    #except:
    #    print("Cosmic integration didn't finish. Better luck next time")
        else:
            rate_det,_,_,_,_ = fastCI.find_detection_rate(output_folder,filename='COMPAS_output.h5')
            rate_per_sys = np.sum(rate_det, axis=1)
        #print(min(rate/mean), max(rate/mean))
            # rate_per_sys[rate_per_sys == 0] = np.min(rate_per_sys[np.nonzero(rate_per_sys)])
            # rate_per_sys = (np.log10(rate_per_sys) + abs(min(np.log10(rate_per_sys))))**3
            mean = np.mean(rate_per_sys)
            return rate_per_sys / mean#, rate_per_sys
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--expl_dir', help = 'Folder name of exploration phase', default = expl_dir)
    ...
    sw_object = genais.Genais(TOTAL_NUM_SYSTEMS, NUM_CPU_CORES, NUM_SYSTEMS_PER_RUN, output_folder, output_filename, debug = debug, run_on_helios = run_on_hpc, mc_only = mc_only)
    ...
    sw_object.initialize(dimensions, interesting_systems, configure_code_run, rejected_systems, bias_CI, update_properties_method = update_properties)
    ...
    sw_object.adapt_without_exploration(expl_dir, n_dimensional_distribution_type = distributions.Gaussian)
    #bias_CI(sw_object, output_folder)
   ## Do selection effects
    #selection_effects(sw)
    #sw_object.refine() #Stroopwafel will draw samples from the adapted distributions
    sw_object.refine(n_dimensional_distribution_type = distributions.Gaussian)
    ...
    sw_object.calculate_weights_of_samples()
