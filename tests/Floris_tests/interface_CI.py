...
### Create hdf5 file of COMPAS data
createHDF5 = True
...
num_systems = 5000                  # Number of binary systems to evolve                                              # Note: overrides pythonSubmit value
...
num_cores = 5                       # Number of cores to parallelize over 
num_per_core = 100                  # Number of binaries per batch
...
def bias_CI(sw, output_folder):
    ...
    try:
        import createhdf5 as postProc
        # Create hdf5 file from the systems sampled in the exploratory phase
        dataRootDir    = output_folder                # Location of root directory of the data     # defaults to '.'
        ...
        h5Name         = 'COMPAS_Output_adapt.h5' # Name of the output h5 file                 # defaults to 'COMPAS_Output.h5'
        filesToCombine = ['SystemParameters', 'CommonEnvelopes', 'DoubleCompactObjects']
        ...
        # Reorder the hdf5 file in increasing seeds
        h5name_ordered='ordered_file.h5'
        postProc.reorder(output_folder, h5Name, filesToCombine, Ordered_COMPAS_name=h5name_ordered)
    except:
        print("Postprocessing didn't finish")

    try:
        import fastCI
        rate_det,_,_,_,_ = fastCI.find_detection_rate(output_folder,filename=h5name_ordered)
...
if __name__ == '__main__':
    ...
    if createHDF5:
        try:
            ...
            postProc.reorder(output_folder, h5Name, filesToCombine)
            postProc.add_weights(output_folder, sw_object.mixture_weight)
