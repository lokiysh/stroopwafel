...
##################################################################
###
### Extra function added to add stroopwafel weights to hdf5 file
###
##################################################################


def reorder(data_dir, Raw_COMPAS_name, groups, Ordered_COMPAS_name='COMPAS_Output_wWeights.h5'):

    File      = h5.File(data_dir + '/'+ Raw_COMPAS_name,'r')
    h_new     = h5.File(data_dir +'/'+ Ordered_COMPAS_name, 'w')

    #for group in ['CommonEnvelopes', 'DoubleCompactObjects', 'RLOF', 'Supernovae', 'SystemParameters']:
    for group in groups:
        print('Copy sort and check ', group)
        h_new.create_group(group)    #################################
        #Get data from the raw/original table 
        OldTable       = File[group]#    #We would like our resulting data to be sorted by SEED (makes life easier)
        indexing = np.argsort(OldTable['SEED'][()])#np.full(len(OldTable['SEED'][()]), True)    ################################
        ...
        for column in OldTable.keys():
            ...
            dataNew    = h_new[group].create_dataset(column, data=data)
            dataNew.attrs['units'] = unit
            #add attribute comment#Always close your files again
    h_new.close()
    File.close()  

def add_weights(data_dir, mixture_weights, Weights_COMPAS_name='COMPAS_Output_wWeights.h5'):

    h_new     = h5.File(data_dir +'/'+ Weights_COMPAS_name, 'r+')
    for group in ['SystemParameters', 'DoubleCompactObjects']:
        UpdatedTable       = h_new[group]#
        print('Adding weights to ', group)
        #################################
        #Add mixture Weights ! 
        if group == 'SystemParameters':
            #Were going to get the weights strtaight from the samples file 
            #seed_pointer = [np.where(allseeds == seed)[0][0] for seed in UpdatedTable['SEED'][()] ]
            data = mixture_weights
    #         data        = samples['mixture_weight'][seed_pointer] #re-order samples.cvs
        elif group == 'DoubleCompactObjects':
            #make sure to point from SYS to DCO
            #and take the data from your SystemParameters (that you shouldve started with!)
            SYS_to_DCO = np.in1d(h_new['SystemParameters']['SEED'][()], h_new['DoubleCompactObjects']['SEED']) #Bool to point SYS to DCO
            data        = h_new['SystemParameters']['mixture_weight'][SYS_to_DCO]#re-order samples.cvs    dataNew     = h_new[group].create_dataset('mixture_weight', data=data)#Always close your files again
        dataNew     = h_new[group].create_dataset('mixture_weight', data=data)#Always close your files again
    h_new.close()
