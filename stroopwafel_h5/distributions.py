...
class Gaussian(NDimensionalDistribution):
    ...
    def run_sampler(self, num_samples, dimensions, consider_rejection_rate = False):
        ...
        num_samples_min = num_samples
        num_samples = int(num_samples * self.biased_weight)
        #if num_samples < int(num_samples_min/4.):
        #    num_samples = int(num_samples_min/4.)
        #if num_samples == 1:
        #    num_samples = 2
        #elif num_samples > int(num_samples * 10):
        ...
        if num_samples == 1:
            current_samples = current_samples[np.newaxis,...]
