...
class Genais:
    ...
    def initialize(self, dimensions, interesting_systems_method, configure_code_run, rejected_systems_method, selection_effects, update_properties_method = None):
        """
        ...
        self.selection_effects = selection_effects
        self.batch_num = 0
        self.num_explored = 0
        self.finished = 0
        self.num_hits = 0
    ...
    def adapt_without_exploration(self, expl_dir, n_dimensional_distribution_type):
        # Read in the fraction of rejected systems from the prior distribution
        # and the number of systems sampled during the previous run
        with open(os.path.join(expl_dir, "log.txt"), 'r') as file:
            logs = file.readlines()
            self.prior_fraction_rejected = float(logs[0].split()[-1])
            self.num_explored = int(float(logs[-1].split()[-1]))
        self.finished = self.num_explored
        self.total_num_systems += self.num_explored # To make sure we really run the number of samples we asked for

        # Next, we want to read in the systems from the previous run
        # and determine which ones will become the new interesting systems
        hits = read_samples(os.path.join(expl_dir, "samples.csv"), self.dimensions, only_hits = True) # read in the original hits
        print("hits:", len(hits))
        # re-define the definition of the target system
        SE_weights = self.selection_effects(expl_dir, genais=True, adapt=True)
        print("CI weights: ", len(SE_weights))
        mask = SE_weights > 1e-3
        hits = np.array(hits)[mask]
        print(len(hits))
        self.num_hits += len(hits)
        self.num_hits_exploratory = self.num_hits
        # Finish by creating the adapted distributions
        [location.transform_variables_to_new_scales() for location in hits]
        average_density_one_dim = 1.0 / np.power(self.num_explored, 1.0 / len(self.dimensions))
        self.adapted_distributions = n_dimensional_distribution_type.draw_distributions(hits, average_density_one_dim)
        for distribution in self.adapted_distributions:
    ...
    def update_distributions(self, locations, tolerance = 0):
        ...
        # weights_normal = np.asarray((pi * mask_hits) / np.sum(qPDF, axis = 1))
        # wn_normalised = (weights_normal / np.sum(weights_normal))[:, None]
        gaussian_weights = np.asarray((pi * mask_hits) / np.sum(qPDF, axis = 1))
        SE_arr = [1] * len(gaussian_weights)
        if self.selection_effects:
            SE_weights = self.selection_effects(self.output_folder, genais=True)
            SE_arr = [0] * len(gaussian_weights)
            indices = np.nonzero(gaussian_weights)[0]
            for index, value in zip(indices, SE_weights[-len(gaussian_weights):]):
                SE_arr[index] = value
        gaussian_weights = np.multiply(gaussian_weights, SE_arr)
        # biased_weights = np.multiply(gaussian_weights, SE_arr)
        # biased_weights_normalised = (biased_weights / np.sum(biased_weights))[:, None]
        ...
        # perplexity = np.exp(entropy(wn_normalised)) / num_samples
        # print("entropy: ", entropy(wn_normalised))
        # print("perplexity: ", perplexity)
        # print("entropy CI :", entropy(weights_normalized))
        # print("perplexity CI :", entropy_change)
        if len(self.entropies) >= 1 and (entropy_change - self.entropies[-1] < MIN_ENTROPY_CHANGE):# and entropy_change - self.entropies[-1] > 0):
            #this is not a good update, probably the last one was the best, so lets revert to it
            print (entropy_change, entropy_change - self.entropies[-1])
    ...
    def calculate_weights_of_samples(self):
        ...
        for generation in range(NUM_GENERATIONS):
            ...
            den += (np.sum(q_PDF, axis = 1) * q_norm) / NUM_GENERATIONS
        weights = pi / den
