...
class Stroopwafel:
    ...
    def __init__(self, total_num_systems, num_batches_in_parallel, num_samples_per_batch, output_folder, output_filename, debug = False, run_on_helios = True, mc_only = False):
        ...
        self.debug = debug                                                            # debug=True, print logs given by external program
        self.run_on_helios = run_on_helios                                            # run_on_helios=True, run the simulation on a remote computer cluster
        self.mc_only = mc_only                                                        # mc_only=True, run a full monte-carlo simulation instead of using adaptive importance sampling
    ...
    def update_fraction_explored(self):
        ...
        self.fraction_explored = 1 - numerator / denominator                                                                 # corresponds to the smallest uncertainty on the target population rate
    ...
    def calculate_mixture_weights(self, n_dimensional_distribution_type, locations):
        ...
        if self.num_explored == self.total_num_systems:                                                                              # if running only monte-carlo, set all mixture weights to 1, since all samples were drawn from the prior distribution
            return
        ...
        for location in locations:
            ...
            location.properties['mixture_weight'] = prior_pdf / Q                                                                    # mixture weight is the probability ratio between drawing sample from prior and mixture distribution
    ...
    def process_batches(self, batches, is_exploration_phase):
        ...
        for batch in batches:
            ...
            if (is_exploration_phase and not self.should_continue_exploring()) or self.finished >= self.total_num_systems or returncode < 0: # if too many batches were created, the excess ones have to be removed
            ...
            if is_exploration_phase:                                                                                                         # during the exploration phase, keep track of the number and fraction of explored systems
                self.num_explored += self.num_samples_per_batch
                self.update_fraction_explored()
            else:
                self.num_to_be_refined -= self.num_samples_per_batch                                                                         # during the refinement phase, keep track of how many systems are left to be sampled
    ...
    def explore(self, intial_pdf):
        ...
        """
        if not self.mc_only:
            self.prior_fraction_rejected = intial_pdf.calculate_rejection_rate(self.update_properties_method, self.rejected_systems_method, self.dimensions)           # check how many drawn samples fall outside the provided property bounds
            print_logs(self.output_folder, "prior_fraction_rejected", self.prior_fraction_rejected)
        else:
            self.prior_fraction_rejected = 0                                                                                                                           # this is not necessary to check for monte-carlo only runs
        while self.should_continue_exploring():                                                                                                                        # as long as we keep exploring...
            batches = []
            for batch in range(self.num_batches_in_parallel):                                                                                                          # we will draw samples for each separate batch
                current_batch = dict()
                ...
                [location.revert_variables_to_original_scales() for location in locations]                                                                             # to prevent confusion, all the property values have to be user-defined in linear scale. here we revert back to these original scales
                ...
                locations = locations[:self.num_samples_per_batch]                                                                                                     # since we drew too many samples, shuffle them to randomize and save the number of systems we actually want
                [location.properties.pop('is_rejected', None) for location in locations]
                current_batch['samples'] = locations
                command = self.configure_code_run(current_batch)                                                                                                       # passes additional command line arguments for each batch
                ...
                self.batch_num = self.batch_num + 1                                                                                                                    # move to next batch
            self.process_batches(batches, True)                                                                                                                        # calculates the progress of the phase/simulation
        if not self.mc_only:
    ...
    def adapt(self, n_dimensional_distribution_type):
        ...
        """
        if self.num_explored != self.total_num_systems:                                                                                                                                                               # if we spend all samples on exploring, there is no room left for refining
            if self.num_hits > 0:                                                                                                                                                                                     # the adapted distributions are created from the target systems, so we need at least 1
    ...
    def refine(self):
        ...
        while self.num_to_be_refined > 0:
            ...
            for batch in range(min(self.num_batches_in_parallel, int(self.num_to_be_refined / self.num_samples_per_batch))):                 # if the refinement phase needs to sample less batches than we run in parallel, make sure we don't run too many
                ...
                for distribution in self.adapted_distributions:                                                                              # repeat the process of sampling and evolving as seen in the exploration phase
